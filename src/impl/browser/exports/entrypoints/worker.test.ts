...
describe('Worker entrypoint', () => {
  ...
  describe('Ping message handling', () => {
    it('should log an error on Ping message', () => {
      const consoleErrorSpy = vi
        ...
        .mockImplementation(() => {})
      workerEntrypoint()
      const message = { type: UpstreamWorkerMessageType.Ping }
      if (!workerScope.onmessage) throw new Error('onmessage is not defined')
      workerScope.onmessage(new MessageEvent('message', { data: message }))
  ...
  describe('Error handling', () => {
    ...
    it('should handle WorkerLocalFirst constructor throwing error', () => {
      const MockWorkerLocalFirst = WorkerLocalFirst as vi.MockedClass<
        typeof WorkerLocalFirst
      >
  ...
  describe('Invalid message handling', () => {
    it('should handle unknown message types gracefully', () => {
      ...
      workerEntrypoint()
      const message = {
        type: 'UNKNOWN_TYPE' as unknown as UpstreamWorkerMessageType,
        data: {}
      }
      if (!workerScope.onmessage) throw new Error('onmessage is not defined')
    ...
    it('should handle messages without type property', () => {
      workerEntrypoint()
      const message = { data: { wsUrl: 'ws://localhost:8080', dbName: 'test-db' } }
      if (!workerScope.onmessage) throw new Error('onmessage is not defined')
    ...
    it('should handle Init message without data property', () => {
      workerEntrypoint()
      const message = { type: UpstreamWorkerMessageType.Init }
      if (!workerScope.onmessage) throw new Error('onmessage is not defined')
