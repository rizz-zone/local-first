...
describe('ping message handling', () => {
	...
	it('handles ping messages correctly', async () => {
		...
		// Ping messages should not cause errors
	...
	it('handles ping messages without initializing first', async () => {
		...
		await new Promise(resolve => setTimeout(resolve, 50))
		// Should handle gracefully even without init
		expect(consoleErrorMock).not.toHaveBeenCalled()
	})
...
describe('port double initialization error handling', () => {
	...
	it('throws PortDoubleInitError on double initialization', async () => {
		...
		// Second initialization should cause error
		userPort.postMessage(initMessage)
	...
	it('handles double init with different parameters', async () => {
		...
		// Second initialization with different params should still error
...
describe('timeout behavior', () => {
	...
	it('sets up timeout on port creation', async () => {
		// Just creating the port should set up a timeout
		// We can't easily test the actual timeout without waiting 60 seconds
		// But we can verify the port was created without errors
	...
	it('ping messages reset timeout without errors', async () => {
		...
		// Send multiple pings to test timeout reset
		for (let i = 0; i < 5; i++) {
			userPort.postMessage(pingMessage)
...
describe('edge cases and robustness', () => {
	...
	it('handles malformed MessageEvent objects', ({ skip }) => {
		...
		// Test with event that has ports but they're not MessagePort objects
	...
	it('handles rapid port connections and disconnections', async ({ skip }) => {
		...
		// Create and connect multiple ports rapidly
		for (let i = 0; i < 10; i++) {
			const channel = new MessageChannel()
		...
		// Should handle all connections and disconnections gracefully
		expect(consoleErrorMock).not.toHaveBeenCalled()
	})
})
...
describe('message serialization edge cases', () => {
	...
	it('handles circular references in transition data', async () => {
		...
		// This should fail during postMessage due to circular reference
		expect(() => {
			userPort.postMessage(transitionMessage)
		}).toThrow()
	})
	...
	it('handles functions in message data', async () => {
		...
		// Functions can't be serialized, should handle gracefully
		expect(() => {
			userPort.postMessage(transitionMessage)
		}).toThrow()
	})
	...
	it('handles symbol properties in message data', async () => {
		...
		// Symbols are not serializable, but object should still work
		expect(() => {
			userPort.postMessage(transitionMessage)
		}).not.toThrow()
...
describe('stress testing', () => {
	...
	it('handles burst of mixed message types', async () => {
		...
		// Should handle the mix without major issues
		// Some errors expected due to double init, but shouldn't crash
	})

	it('handles very rapid message sending', async () => {
		// Send 100 ping messages as fast as possible
		for (let i = 0; i < 100; i++) {
			userPort.postMessage({
				type: UpstreamWorkerMessageType.Ping
			})
